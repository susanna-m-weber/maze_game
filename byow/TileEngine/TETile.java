...
public class TETile implements Serializable {
    ...
    private int changeChannel(int v, double d) {
        int newValue =  Math.min(255, Math.max(0, (int) (v * d)));
        return newValue;
    }

    public TETile changeBrightness(double d) {
        TETile ret = new TETile(this);
        ret.brightness = d;
        return ret;
    }
    ...
    /**
     * Creates a copy of TETile t, except with given textColor and bgColor
     * @param t tile to copy
     * @param textColor foreground color for tile copy
     */
    public TETile(TETile t, Color textColor, Color bgColor) {
        this(t.character, textColor, bgColor, t.description, t.filepath);
        this.blocks = t.blocks;
        this.brightness = t.brightness;
    }

    Color adjustBrightness(Color c) {
        if (brightness > 1.0) {
            brightness = 1.0;
        }
        int r = changeChannel(c.getRed(), brightness);
        int g = changeChannel(c.getGreen(), brightness);
        int b = changeChannel(c.getBlue(), brightness);
    ...
    public void draw(double x, double y) {
        ...
        if (brightness != 1.0) {
            Color bg = adjustBrightness(backgroundColor);
            Color tx = adjustBrightness(textColor);
            StdDraw.setPenColor(bg);
            StdDraw.filledSquare(x + 0.5, y + 0.5, 0.5);
            StdDraw.setPenColor(tx);
    ...
    public void setBlocks(boolean newBlocking) {
        blocks = newBlocking;
    }
}
